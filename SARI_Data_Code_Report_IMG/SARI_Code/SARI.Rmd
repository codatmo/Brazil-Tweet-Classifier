...
author: "André Luis Marques Ferreira dos Santos | andrelmfsantos@gmail.com"
output:
  html_document:
    df_print: paged
---
...
Summary SUS
```{r, echo=FALSE}
dftab <- psych::describe(df)
print(dftab)
write.csv(df, "sars_ts.csv")
write.csv(dftab, "summarySUS.csv")
```
...
```{r, echo=FALSE}
df1tab <- psych::describe(df1)
print(df1tab)
write.csv(df1, "twitter_ts.csv")
write.csv(df1tab, "summaryTwitterU9.csv")
```
```{r, echo=TRUE}
twitter_plotU9 <-
...
twitter_plotU9 + theme(legend.position = "bottom")

fig2 <- ggplotly(twitter_plotU9)
fig2

ggsave("twitter_plotU9.png", width = 32, height = 18, units = "cm", dpi = 300)
print(twitter_plotU9)
```
#Twitter 2019-2020
...
twitter_Liver <-
  ...
  #stat_smooth(method = "lm", formula = y ~ x, size = 1, col = "red")
twitter_Liver + theme(legend.position = "bottom")

fig4 <- ggplotly(twitter_Liver)
fig4

ggsave("twitter_Liver.png", width = 32, height = 18, units = "cm", dpi = 300)
print(twitter_Liver)
```
Summary Twitter Liverpool
```{r, echo=FALSE}
dt2tab <- psych::describe(dt2)
print(dt2tab)
write.csv(dt2, "twitter_Liverpool.csv")
write.csv(dt2tab, "summaryTwitterLiver.csv")
```
...
twitter_Gripe <-
  ...
  #stat_smooth(method = "lm", formula = y ~ x, size = 1, col = "red")
twitter_Gripe + theme(legend.position = "bottom")

fig5 <- ggplotly(twitter_Gripe)
fig5

ggsave("twitter_Gripe.png", width = 32, height = 18, units = "cm", dpi = 300)
print(twitter_Gripe)
```
### SARI + Twitter
```{r, echo=FALSE}
...
dfs <- na.omit(dfs)
# Save csv file---------------------------------------------------------------
write.csv(dfs, "sars_twitterU9.csv")
```

Summary Twitter Liverpool
```{r, echo=FALSE}
dfstab <- psych::describe(dfs)
print(dfstab)
write.csv(dfs, "SARS-Twitter.csv")
write.csv(dfstab, "summarySARS-Twitter.csv")
```
...
ggplot(d) +
  geom_line(aes_string(x, y1), color = "black", size = 1) + 
  geom_line(aes_string(x, y2), color = "red", size = 1) +
  scale_y_continuous("Tweets(Gripe)",sec.axis = sec_axis(trans=trans, name=y2))+
  #scale_x_continuous("Year 2020") +
  theme(axis.line.y.right = element_line(color = "red"), 
        axis.ticks.y.right = element_line(color = "red"),
        axis.text.y.right = element_text(color = "red"), 
...
fig7 <- ggplotly(covid_gripe)
fig7
print(covid_gripe)
ggsave("covid_gripe.png", width = 32, height = 18, units = "cm", dpi = 300)
```
